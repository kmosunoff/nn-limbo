        ...
        for y in range(out_height):
            for x in range(out_width):
                result[:, y: y + filter_size, x: x + filter_size, :] += \
                    (d_out[:, y, x, :] @ W.T).reshape((batch_size, filter_size, filter_size, input_channels))
                I = self.X[:, y: y + filter_size, x: x + filter_size, :] \
                    .reshape((batch_size, filter_size * filter_size * input_channels))
...
class MaxPoolingLayer:
    ...
    def forward(self, X):
        ...
        for y in range(out_height):
            for x in range(out_width):
                current_area = X[:, y * self.stride: y * self.stride + self.pool_size,
                          x * self.stride: x * self.stride + self.pool_size, :]
                result[:, y, x, :] = np.max(current_area, axis=(1, 2))
    ...
    def backward(self, d_out):
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                current_area = self.X[:, y * self.stride: y * self.stride + self.pool_size,
                            x * self.stride: x * self.stride + self.pool_size, :]
                mask = (current_area == np.max(current_area, axis=(1, 2))[:, np.newaxis, np.newaxis, :])
